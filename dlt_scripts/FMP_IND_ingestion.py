# FMP ingestion script for IND COMPANIES
...
TICKER_FILE = Path("/opt/airflow/config/smp500_ingestion/FMP_IND_ticker.txt")
...
DB_TABLE = "raw_fmp_ind_prices_daily"
S3_BUCKET = os.getenv("S3_BUCKET")
S3_PREFIX = "smp500_ingestion/fmp_ind_prices_daily"
...
def upload_df_to_s3(df):
    s3 = boto3.client("s3")
    file_name = f"fmp_ind_prices_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
